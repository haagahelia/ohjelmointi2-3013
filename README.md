...
Voit tallentaa projektin lähdekoodit itsellesi GitHub-palvelusta monilla eri tavoilla. Yksinkertaisimmillaan voit tallentaa sen [zip-pakettina](archive/master.zip), jonka tuot itsellesi Eclipseen import-ominaisuudella. 
...
`javax.servlet.http.HttpServletResponse` on olio, jonka kautta voidaan lähettää vastaus saatuun pyyntöön. Vastaus voidaan esim. kirjoittaa `System.out.println()`-tyyliin pyytämällä `HttpServletResponse`-oliolta writer-olio `getWriter()`-metodilla, ja kutsumalla sen `println`-metodia:
